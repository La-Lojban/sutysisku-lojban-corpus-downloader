...
//downloads dumps for languages selected and converts to dexie format

downloadDumps();

export async function downloadDumps() {
  await updateXmlDumps();
}

async function updateXmlDumps() {
  fs.copySync('./src/prebuilt_dumps', './data/dumps');
  ...
  for (const language of predefinedLangs.concat(langs)) {
    try {
      await ningauPaLaSutysisku(language);
    } catch (error: any) {
...
function splitToChunks(array: any[], parts: number, tegerna: string): any[] {
...
function workerGenerateChunk({ segerna, chunk, index }: { segerna: string; chunk: string; index: number }) {
  const outp = {
    formatName: 'dexie',
    formatVersion: 1,
    data: {
...
async function ningauLeDeksiSutysisku({ json, segerna }: { json: Dict; segerna: string }) {
  let arr = [];
...
async function ningauPaLaSutysisku(segerna: string) {
